...
class CareerPathAdvisorApp:
    def __init__(self, master):
        ...
        self.master.geometry("600x800")  # Set the window size
        self.master.configure(bg="darkgray")  # Set background color
        self.master.iconbitmap("P:\IDT\IDT\IDT\hello.ico") #Logo image
        ...
        software_engineer = CareerPath(
            "Software Engineer",
            [" |Programming Languages-\n    -> JavaScript\n    -> Python\n    -> C++\n    -> Rust\n    -> Go\n    -> Swift", " |Algorithm Design", " |Database Management\n    ->SQL\n    -> Data Analsis\n    -> Data Integtity"],
        ...
        # Create UI elements
        self.label = tk.Label(master, text="Select a Career Path:", font=heading_font, fg="darkblue", bg="darkgray")
        self.label.pack()
        ...
        for path in self.advisor.career_paths:
            button = tk.Button(master, text=path.name, width=40, font=("Verdana", 15),
                               relief=tk.RAISED, bd=3, command=lambda p=path: self.show_details(p), bg="#d9d9d9", activebackground="gray")
            button.pack(pady=5)
