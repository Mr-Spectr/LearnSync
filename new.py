...
class CareerPathAdvisorApp:
    ...
    def logout(self):
        logout_window = tk.Toplevel(self.master)
        app = LogoutWindow(logout_window, self.master, self.username)
...
class LogoutWindow:
    def __init__(self, master, login_root, username):
        self.master = master
        self.login_root = login_root
        self.username = username
        self.master.title("Logout Window")
        ...
        # Widgets
        self.logout_label = tk.Label(master, text="What would you like to do?", font=("Helvetica", 30),
                                     bg="lightgray")
        self.logout_label.pack(pady=(20, 10))

        self.logout_button = tk.Button(master, text="Logout", command=self.logout, font=("Helvetica", 20),
                                       bg="green", fg="white", height=1, width=10)
        self.delete_account_button = tk.Button(master, text="Delete Account", command=self.delete_account,
                                               font=("Helvetica", 16), bg="red", fg="white", height=1, width=15)

        self.logout_button.pack(pady=20)
        self.delete_account_button.pack(pady=20)

    def logout(self):
        self.master.destroy()  # Close the logout window
        self.login_root.deiconify()  # Show the first login window
